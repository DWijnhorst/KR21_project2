...
#EXPERIMENT 1: the effect of min-fill vs min-order on marginalization time.
    #min fill
    for i in range(4):
        # Test net temporary
        # NET  = BNReasoner("testing/dog_problem.BIFXML")
        #script for getting right network
        NET = BNReasoner("experiments/experimentnet%s.BIFXML" %i)
        Vars = NET.Get_Vars()
        print('Vars = ', Vars)
        # The run time of the ordering itself
        ordering_start_time = time.time()

        ordering = NET.Ordering(Vars, 'min-fill')
        ordering_runtime = time.time() - ordering_start_time
        print('ordering_runtime =',ordering_runtime)
        # The run time 
        start_time = time.time()
        for i in ordering:
            NET.Marginalization(i)
        runtime = time.time() - start_time
        print('runtime min-fill =' ,runtime)
    #min-degree
    for i in range(4):
            # Test net temporary
            # NET  = BNReasoner("testing/dog_problem.BIFXML")
            ...
            ordering = NET.Ordering(Vars, 'min-degree')
            ordering_runtime = time.time() - ordering_start_time
            print('ordering_runtime =',ordering_runtime)
            # The run time 
            start_time = time.time()
            for i in ordering:
                NET.Marginalization(i)
            runtime = time.time() - start_time
            print('runtime min-degree =' ,runtime)
# EXPERIMENT 2: The effect of large vs smaller networks on Marginalization:
